...
void read_transaction_file(char * path) {
    ...
    while(getline(&line, &len, fd) != -1) {
        ...
        //Modify ram size
            ...
            if(arguments[1] == NULL || arguments[2] == NULL) {
                printf("Warning : Invalid VM number or ram size on line %d\n", line_count);
                continue;
            }
            ...
            //Use a struct to pass the VM number and the ram size to the thread
            RANGE *range = malloc(sizeof(RANGE));
            range->start = atoi(arguments[1]);
            range->end = atoi(arguments[2]);

            if(!is_thread_list_full()) {
                pthread_create(&threads[thread_count], NULL, modify_ram, (void *) range);
                thread_count++;
