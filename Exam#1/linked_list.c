...
void *add_vm() {
    ...
    new_vm_infos->ram_size = 0;
    new_vm_infos->ram = NULL;
...
void *print_vm(void *range) {
    ...
    //Iterate through the list until the end
        ...
        //If the VM number is between the range, print it
            ...
            printf(" : Busy = %d,", list->vm_infos->busy);
            printf(" RAM size = %d\n", list->vm_infos->ram_size);
        }
...
void *modify_ram(void *args) {

    RANGE numbers = *(RANGE *) args;
    int vm_number = numbers.start;
    int ram_size = numbers.end;
    ...
    while(list != NULL) {
        if(list->vm_infos->number == vm_number) {
            list->vm_infos->ram_size = ram_size;
            list->vm_infos->ram = malloc(ram_size);
            break;
        }
